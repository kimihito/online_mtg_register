class EventsController < ApplicationController
  ...
  def create
    ...
    if @event.start_at.future? == false
      redirect_to( mypage_path, :notice => 'Event should be in the future') and return
    end
    ...
    redirect_to( mypage_path, :notice => ("Event #{link} was successfully created.").html_safe) and return
    ...
    if @event.start_at > @event.end_at
      redirect_to( mypage_path, :notice => 'Start time should be equal or greater than end time') and return
    end
    @event.save
    link ="<a href=events/#{@event.id}>#{@event.name}</a>"
    redirect_to( mypage_path, :notice => ("Event #{link} was successfully created.").html_safe) and return
  ...
  def destroy
    ...
    respond_to do |format|
      format.html {redirect_to( mypage_path, :notice => 'Event removed.')}
      format.xml  { head :ok }
    end
  end
